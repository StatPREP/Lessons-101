...
1. Open up the [jittering Little App`](`r jitter_app$url`). Select `NHANES` as the data frame,  with `sex` as the response variable and `education` as the explanatory variable.  Leave the sample size a n = 50.
...
Version 0.2, `r Sys.Date()`, Danny Kaplan, [Word version](`r paste0("/word-versions/",tools::file_path_sans_ext(knitr::current_input()), ".docx")`) 